...
def runK8(image, branchName, config, axis, steps=config.steps) {
    ...
    def limits = image.limits ?: getConfigVal(config, ['kubernetes', 'limits'], "{memory: 8Gi, cpu: 4000m, ephemeral-storage: 5Gi}")
    def requests = image.requests ?: getConfigVal(config, ['kubernetes', 'requests'], "{memory: 8Gi, cpu: 4000m, ephemeral-storage:")
